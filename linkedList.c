...
void* find_list(void *clave, LinkedList *l) {
    Node *nodoaux = NULL;
...
/**
 * @page destroy_all_nodes_list \b destroy_all_nodes_list
 *
 * @brief Call the destroy all nodes of a list function.
 * @param l A list.
 * @return int.
 *
 * @section SYNOPSIS
 ...
 *  \b lib/libLista.a 
 *
 *  \b int \b destroy_all_nodes_list (\b Lista * l \b)
 * 
 * @section descripcion DESCRIPCIÓN
 *
 * This function call the deletes an element elem of the lis l if this element is in the list function.
 * 
 * Recives:
 *    - l The list.
 *
 ...
 * Jorge Guillen (jorge.guillen@estudiante.uam.es)
*/
int destroy_all_nodes_list (LinkedList * l){

    return destroy_all_nodes(l->first);    
}
 ...
 *  \b lib/libLista.a 
 *
 *  \b int \b destroy_all_nodes (\b Nodo * node \b)
 * 
 * @section descripcion DESCRIPCIÓN
 *
 * Deletes the node of a list an al the "next" nodes of that node.
...
*/
int destroy_all_nodes (Node * node){
    if (node->next != NULL){
        destroy_all_nodes(node->next);     
    }
    destroy_node(node);
    node=NULL;
    return 0;
